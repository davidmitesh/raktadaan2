...
app.post('/donorproof',(req,res)=>{
    var name = './images/'+req.body.id+'proof'+'.jpg';
    var img = req.body.image;
    var realFile = Buffer.from(img,"base64");
    fs.writeFile(name, realFile, function(err) {
        if(err)
    ...
    User.findOneAndUpdate({"user_id":parseInt(req.body.id)},{donation_claim: true,$push:{donated:{name:req.body.name,number:req.body.number}}},(err,results)=>{
        // res.redirect('/');
